...
async def lifespan(app_: FastAPI):
    ...
    async with _trigger_mcp_inner.lifespan(app_), _doppler_mcp_inner.lifespan(app_):
        ...
        try:
            ...
            # Idempotent + self-healing, serialized across replicas by a session advisory lock; a failure
            # re-raises → the boot fails → Railway keeps the prior healthy deploy on traffic rather than
            # promoting a version whose code outruns the live schema. Closes the code↔schema drift that
            # 500'd document-payment reads (the rail column added in committed DDL but never applied to
            # prod). See src/migrate.py.
