"""Archetype-version render — STANDALONE path for the docraptor-to-documenso *template* family.

A self-contained surface with NO ties to the agreement-document (document-only) pathway: its own content
...
Content tree (under ``apps/edge_api/content/active-operators/``):
    ...
    <path>/<archetype>/<version>/global_agreement_content/{manifest.json, *.html, styles/*.css}

e.g. ``docraptor-to-documenso-template/term-only/v1/global_agreement_content/``. The operator selects
