"""Content catalog for the archetype-version path — discover + safely resolve (brand, path,
archetype, version) tuples to a repo-resident template directory.

A template is any ``<brand>/<path>/<archetype>/<version>/global_agreement_content/manifest.json``
...
# apps/edge_api/src/archetype_versions/catalog.py -> parents[2] == apps/edge_api
_CONTENT_ROOT = (pathlib.Path(__file__).resolve().parents[2] / "content").resolve()
_DEFAULT_BRAND = "active-operators"
_CONTENT_SUBDIR = "global_agreement_content"
...
def resolve(path: str, archetype: str, version: str, *, brand: str = _DEFAULT_BRAND) -> pathlib.Path:
    """The ``global_agreement_content`` dir for a (brand, path, archetype, version) tuple. Raises
