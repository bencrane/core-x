...
Mounted under ``/mcp/trigger`` on the edge_api app via FastMCP's ``http_app()``
(see ``apps/edge_api/main.py``). Managed agents connect via standard MCP HTTP
transport; transport auth is the shared MCP bearer (``DMAAS_MCP_BEARER_TOKEN``)
enforced by the ASGI wrapper at the mount boundary (``src/mcp_bearer.py``).

Each tool is a thin wrapper around ``trigger_dev_client``, which talks to the
Trigger.dev v3 Management API (https://api.trigger.dev) using ``TRIGGER_SECRET_KEY``
— scoped to the core-x Trigger.dev project. Tool calls that fail (misconfigured
key, API error) return a structured ``{"error": ...}`` dict rather than raising,
so a single bad call never tears down the session.
"""
...
from ..services import trigger_dev_client as td

logger = logging.getLogger(__name__)

mcp = FastMCP(
    name="core-x Trigger.dev",
    instructions=(
        "Control Trigger.dev background tasks in the core-x project: enqueue a "
        "task run (trigger_task), check status/output (get_run), list recent runs "
...
async def trigger_task(
    ...
    """Enqueue a run of the Trigger.dev task `task_identifier` in the core-x project.
...
async def list_runs(
    ...
    """List recent runs in the core-x Trigger.dev project, newest first. Optionally
