"""Minimal async Trigger.dev v3 Management API client.

Backs the ``/mcp/trigger`` MCP mount (task + run control: ``trigger_task``,
``get_run``, ``list_runs``, ``cancel_run``) plus the schedule-lifecycle helpers
(``create_schedule`` / ``delete_schedule``). Copied verbatim from hq-x
``app/services/trigger_dev_client.py`` — it carries no internal hq-x coupling
(``os`` + ``httpx`` only).

Auth: ``TRIGGER_SECRET_KEY`` (Doppler ``core-x/prd``) — scopes to the core-x
Trigger.dev project (``proj_pakdcffjbeiwcixcoepb``, per ``trigger.config.ts``),
where the agent's pipeline / hydration tasks deploy.

Endpoints used:
  POST   https://api.trigger.dev/api/v1/tasks/{id}/trigger   enqueue a run
  GET    https://api.trigger.dev/api/v3/runs/{id}            run status/output
  GET    https://api.trigger.dev/api/v1/runs                 list recent runs
  POST   https://api.trigger.dev/api/v2/runs/{id}/cancel     cancel a run
  POST   https://api.trigger.dev/api/v1/schedules            create schedule
  DELETE https://api.trigger.dev/api/v1/schedules/{id}       delete schedule
"""
