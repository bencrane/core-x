...
def _api_key() -> str:
    secret = os.environ.get("ANTHROPIC_MANAGED_AGENTS_API_KEY")
    if secret is None:
...
def _agent_id() -> str:
    if not os.environ.get("MANAGED_AGENT_ID_GTM"):
        raise ManagedAgentsNotConfiguredError(
            status_code=None,
            message="MANAGED_AGENT_ID_GTM is not configured (Doppler)",
        )
    return os.environ.get("MANAGED_AGENT_ID_GTM")


def _environment_id() -> str:
    if not os.environ.get("MANAGED_ENVIRONMENT_ID_GTM"):
        raise ManagedAgentsNotConfiguredError(
            status_code=None,
            message="MANAGED_ENVIRONMENT_ID_GTM is not configured (Doppler)",
        )
    return os.environ.get("MANAGED_ENVIRONMENT_ID_GTM")
...
def _default_vault_ids() -> list[str]:
    ...
    """
    out: list[str] = []
    if os.environ.get("MANAGED_VAULT_ID_GTM_MCP"):
        out.append(os.environ.get("MANAGED_VAULT_ID_GTM_MCP"))
    return out
