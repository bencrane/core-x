"""business.engagement_content persistence — psycopg async.
...
async def create_draft(
    ...
    *,
    ...
    sql = f"""
        INSERT INTO business.engagement_content
...
async def update_draft(
    ...
    *,
    ...
    sql = (
        f"UPDATE business.engagement_content SET {', '.join(sets)} "
...
async def get(conn, id: str) -> dict[str, Any] | None:
    async with conn.cursor(row_factory=dict_row) as cur:
        await cur.execute(
            f"SELECT {_SELECT_COLS} FROM business.engagement_content WHERE id = %s", (id,)
        )
...
async def get_published_by_slug(conn, slug: str) -> dict[str, Any] | None:
    ...
    async with conn.cursor(row_factory=dict_row) as cur:
        await cur.execute(
            f"SELECT {_SELECT_COLS} FROM business.engagement_content "
...
async def list_all(
    ...
    *,
    ...
    async with conn.cursor(row_factory=dict_row) as cur:
        await cur.execute(
            f"SELECT {_SELECT_COLS_PT} FROM business.engagement_content pt {join} {where} "
...
async def publish(
    ...
    sql = f"""
        UPDATE business.engagement_content
