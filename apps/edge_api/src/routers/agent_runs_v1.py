...
Mounts at ``/api/v1/agent-runs``. Auth is edge_api's service-token gate
(``require_service_token`` / ``EDGE_API_SERVICE_TOKEN``); the caller
(``platform-api``) has already validated the customer's Supabase JWT
upstream and forwards the resolved ``user_id`` in the request body for the
...
from ..services import managed_agents
...
async def create_agent_run(
    ...
    row = await _insert_agent_run(
        ...
        agent_id=os.environ.get("MANAGED_AGENT_ID_GTM") or "",
        environment_id=os.environ.get("MANAGED_ENVIRONMENT_ID_GTM") or "",
