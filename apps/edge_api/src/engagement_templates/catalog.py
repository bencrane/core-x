"""Content catalog for the engagement-template path — discover + safely resolve (path, archetype,
version) triples to a repo-resident template directory.

A template is any ``<path>/<archetype>/<version>/global_engagement_content/manifest.json`` under the
active-operators content root. The selection segments arrive from the operator (via the BFF), so each
is validated against a strict allowlist AND the resolved directory is confirmed to live inside the
root — defense-in-depth against path traversal.
"""
...
_AO_ROOT = (pathlib.Path(__file__).resolve().parents[2] / "content" / "active-operators").resolve()
...
def list_templates() -> list[TemplateEntry]:
    """Every selectable (path, archetype, version) under the content root, sorted. A family whose
    manifest does not sit at the ``<path>/<archetype>/<version>/`` depth is naturally excluded."""
    out: list[TemplateEntry] = []
    if not _AO_ROOT.is_dir():
        return out
    for path_dir in sorted(p for p in _AO_ROOT.iterdir() if p.is_dir()):
...
def resolve(path: str, archetype: str, version: str) -> pathlib.Path:
    """The ``global_engagement_content`` dir for a triple. Raises ``CatalogError`` on a bad segment,
    a path that escapes the root, or a missing manifest."""
    for seg in (path, archetype, version):
        if not _SAFE_SEG.match(seg):
            raise CatalogError(f"invalid selection segment: {seg!r}")
    content_dir = (_AO_ROOT / path / archetype / version / _CONTENT_SUBDIR).resolve()
    # defense-in-depth: the resolved dir MUST sit inside the content root
    if _AO_ROOT not in content_dir.parents:
        raise CatalogError("resolved path escapes the content root")
    if not (content_dir / _MANIFEST).is_file():
        raise CatalogError(f"unknown template: {path}/{archetype}/{version}")
    return content_dir
