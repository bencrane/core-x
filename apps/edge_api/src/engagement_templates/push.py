"""Render an engagement-content source to PDF and PUSH it to Documenso as a TEMPLATE.
...
# extension point (resolve business.global_engagement_content.markdown by slug → branded HTML).
REPO_HTML = "repo-html"
...
async def render_and_push(
    *,
    ...
    if source_kind != REPO_HTML:
        # db-markdown would resolve business.global_engagement_content.markdown by slug and render via
    ...
    if store_copy:
        ...
        try:
            ...
            logger.warning("engagement-template push: R2 audit copy failed (non-fatal): %s", exc)
            pdf_r2_key = None
