...
-- ── DIGITAL EVENT archetype seeds (ruled 2026-07-21) ─────────────────────────────────────────
-- Version v1 (single Event; duration fixed text; attendance min-max range; refund-by-default
-- settlement priced at fee/min; supplemental at Principal's discretion, displacing only if
-- granted AND accepted).
INSERT INTO gc.global_agreement_archetype_versions (archetype_id, content_path, name, version)
SELECT a.id,
       'government-contracted/docraptor-to-documenso-template/originated-digital-event/v1',
       'Strategic Origination Agreement (Originated Digital Event) — One-Page Body',
       'v1'
  FROM gc.global_agreement_archetypes a
 WHERE a.key = 'digital_event'
ON CONFLICT (content_path) DO NOTHING;

-- Requirements: 13 fields in document order. AttendeePriceMin is DERIVED = PrepaidFee /
-- MinAttendeesNum (per-attendee price at the MIN side; prices the automatic prorated refund).
INSERT INTO gc.global_agreement_archetype_variables
    (archetype_id, ordinal, documenso_field_label_to_use, field_type, value_source)
SELECT a.id, v.ordinal, v.label, v.field_type, v.value_source
  FROM gc.global_agreement_archetypes a,
       (VALUES
            ...
            (2,  'Legal Entity Name', 'text',      'entered'),
            (3,  'D/B/A Name',        'text',      'entered'),
            (4,  'PrepaidFee',        'text',      'entered'),  -- machine-read charge key (FEE_KEYS)
            (5,  'MinAttendeesNum',   'text',      'entered'),
            (6,  'MaxAttendeesNum',   'text',      'entered'),
            (7,  'AttendeePriceMin',  'text',      'derived'),  -- = PrepaidFee / MinAttendeesNum
            (8,  'Signature',         'signature', 'auto'),     -- Principal column
            (9,  'Date',              'date',      'auto'),     -- Principal sig date
            (10, 'Signature',         'signature', 'auto'),     -- Participant column
            (11, 'Full Name',         'text',      'entered'),
            (12, 'Title',             'text',      'entered'),
            (13, 'Date',              'date',      'auto')      -- Participant sig date
       ) AS v(ordinal, label, field_type, value_source)
 WHERE a.key = 'digital_event'
ON CONFLICT (archetype_id, ordinal) DO NOTHING;

-- Hardcoded ceremony sides (dates + signatures never carry Required/Read-Only; sides fixed).
UPDATE gc.global_agreement_archetype_variables v SET recipient = m.r
  FROM (VALUES (1, 'principal'), (8, 'principal'), (9, 'principal'),
               (10, 'participant'), (13, 'participant')) AS m(o, r),
       gc.global_agreement_archetypes a
 WHERE a.key = 'digital_event' AND v.archetype_id = a.id AND v.ordinal = m.o
   AND v.recipient IS DISTINCT FROM m.r;

-- Context paragraphs (decision context for the field-rules surface; not the legal SoR).
INSERT INTO gc.global_agreement_archetype_paragraphs (archetype_id, ordinal, heading, body)
SELECT a.id, p.ordinal, p.heading, p.body
  FROM gc.global_agreement_archetypes a,
       (VALUES
            (1, 'Preamble',
             'This Strategic Origination Agreement is entered into and made effective as of ________ (the “Effective Date”), by and between Rare Structure LLC (“Principal”), and ________________ (d/b/a ________________) (“Participant”).'),
            (2, '§2.1 Prepaid Event Fee & Attendance Range',
             'Participant shall pay an upfront fee of ________ (the “Prepaid Event Fee”), due in full upon execution. The Prepaid Event Fee secures the Event with no fewer than ____ live attendees (the “Minimum Attendance”) and up to ____ live attendees (the “Maximum Attendance”), establishing a unit price of ________ per attendee (the “Per-Attendee Price”, equal to the Prepaid Event Fee divided by the Minimum Attendance).'),
            (3, 'Execution — Principal column',
             'Principal signature block: signature line, Name: Benjamin J. Crane (pre-set), Title: Managing Director (pre-set), Date: ________.'),
            (4, 'Execution — Participant column',
             'Participant signature block: signature line, Name: ________, Title: ________, Date: ________.')
       ) AS p(ordinal, heading, body)
 WHERE a.key = 'digital_event'
ON CONFLICT (archetype_id, ordinal) DO NOTHING;
-- Variable → paragraph mapping (fill-only).
UPDATE gc.global_agreement_archetype_variables v SET paragraph_ordinal = m.para
  FROM (VALUES (1,1),(2,1),(3,1),(4,2),(5,2),(6,2),(7,2),(8,3),(9,3),(10,4),(11,4),(12,4),(13,4))
       AS m(ord, para),
       gc.global_agreement_archetypes a
 WHERE a.key = 'digital_event' AND v.archetype_id = a.id AND v.ordinal = m.ord
   AND v.paragraph_ordinal IS NULL;
