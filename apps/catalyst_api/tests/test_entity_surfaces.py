...
exactly like the live Lance schemas: ``sam_entity_master``, ``sam_pocs``,
``entity_profile_gold``, and ``usaspending/award_search``.
"""
...
def test_sam_profile_derives_status_days_and_secondary_naics():
    entity = {
        ...
        "registration_date": dt.date(2019, 1, 1), "activation_date": dt.date(2019, 1, 3),
        "expiration_date": dt.date(2026, 9, 1),
        "primary_naics": "333120", "naics_codes": ["333120", "333111", "423810"],
        "psc_codes": ["3040", "H170"], "business_types": "2X~A6~XS",
        "physical_city": "St. Martin", "physical_state": "MN", "physical_zip5": "56376",
    }
    pocs = [
    ...
    assert d["registrationStatus"] == "active"               # expiry in the future
    ...
    assert d["businessTypesRaw"] == "2X~A6~XS"               # raw, NOT parsed
...
def test_sam_profile_expired_when_past():
    entity = {"uei": "WBBSNBA9GBL7", "expiration_date": dt.date(2025, 1, 1), "naics_codes": []}
    d = SamProfileResponse.from_row(entity, [], TODAY).model_dump(by_alias=True)
    assert d["registrationStatus"] == "expired"
