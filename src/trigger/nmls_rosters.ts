 ...
 *   2. ingest_target  — fanned out per target in PARALLEL (distinct Lance datasets → no
 *      shared-writer conflict): DuckDB read_csv/read_xlsx → Lance on R2 + scalar indexes.
 *
...
export const nmlsRostersIngest = task({
  ...
  // acquire (one scrape) + a parallel fan-out of small overwrites + index builds; the
  ...
  run: async (payload: { as_of?: string }) => {
    ...
    // Phase 2 — fan out ingest per target in parallel (distinct Lance datasets — no conflict).
