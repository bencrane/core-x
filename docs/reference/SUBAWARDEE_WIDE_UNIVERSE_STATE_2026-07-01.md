...
| **FSRS amount guard** | `subaward_amount` is notoriously dirty — one mis-keyed line ($39.16T, verified) dominated every dollar sum. **Guarded 2026-07-01:** `\|amount\| > $1B` NULLed at ingest in both sub-rail builders (`SUBAWARD_AMOUNT_MAX`, 26 lines). Counts/medians were always clean and are the ranking signal; sums are now sane ($361.3B combo table, $1.17T work_profile sub rollup). |
...
- **FSRS amount corruption — ✅ GUARDED 2026-07-01.** `subaward_amount` is notoriously dirty; a single mis-keyed line (`541330|H914`, CPI SATCOM, **$39.16T** — 98.9% of the raw $39.6T total) dominated every dollar aggregate. Fixed by NULLing `|amount| > $1B` (`SUBAWARD_AMOUNT_MAX`, default `1e9`) at ingest in **both** `scripts/build_subaward_naics_psc_wide.py` and `pipelines/usaspending/subawardee_work_profile.py` (build_wide). A mis-key is an *unknown*, not a capped value → NULL, not clamp; the line survives for counts. 26 lines nulled; post-fix combo total **$361.3B**, max line **$980.5M**, `541330|H914` **$57.28M**. **Ranking is count-based** (`n_primes` / `confidence·lift` / `median_subaward_amt`), so lane order was never affected — only displayed `total_*_amt` / `sub_amount_5y` figures. Threshold caveat: nulls a handful of borderline-plausible $1–5.5B defense megasubs alongside the mis-keys (conservative — tune via env). FPDS prime obligations are a separate clean feed, not guarded.
