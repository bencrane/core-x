...
| **FSRS amount guard** | `subaward_amount` is notoriously dirty — one mis-keyed line ($39.16T) dominated every dollar sum. **Guarded 2026-07-01:** `\|amount\| > $20B` NULLed at ingest in both sub-rail builders (`SUBAWARD_AMOUNT_MAX=2e10`) — the threshold sits in the empty gap between the largest REAL line ($5.5B) and the impossible one, so **only the 1 mis-key is nulled; all 24 real $1–5.5B megasubs are kept.** Counts/medians were always clean and are the ranking signal; sums are now sane ($437.8B combo table, $1.31T work_profile sub rollup). |
...
- **FSRS amount corruption — ✅ GUARDED 2026-07-01.** `subaward_amount` is notoriously dirty; a single mis-keyed line (`541330|H914`, CPI SATCOM, **$39.16T** — 98.9% of the raw $39.6T total) dominated every dollar aggregate. Fixed by NULLing `|amount| > $20B` (`SUBAWARD_AMOUNT_MAX`, default `2e10`) at ingest in **both** `scripts/build_subaward_naics_psc_wide.py` and `pipelines/usaspending/subawardee_work_profile.py` (build_wide). A mis-key is an *unknown*, not a capped value → NULL, not clamp; the line survives for counts. **The threshold sits in the empty gap between the largest real line ($5.5B, a Lockheed megasub) and the $39.16T impossible one, so only the 1 gross mis-key is nulled and all 24 plausible $1–5.5B megasubs are kept** (an earlier $1B pass nulled all 26 lines >$1B — too aggressive, corrected 2026-07-02). Post-fix combo total **$437.8B**, max line **$5.5B**, `541330|H914` **$57.28M**, work_profile max `sub_amount_5y` **$12.39B**. **Ranking is count-based** (`n_primes` / `confidence·lift` / `median_subaward_amt`), so lane order was never affected — only displayed `total_*_amt` / `sub_amount_5y` figures. FPDS prime obligations are a separate clean feed, not guarded.
