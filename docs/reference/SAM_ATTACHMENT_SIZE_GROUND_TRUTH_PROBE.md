...
- Sampling is deterministic (md5(resource_id‖seed)); re-running with the same seed reproduces the exact URL set. Raw per-URL results (ephemeral): `/tmp/sam_size_probe_raw.jsonl` — regenerable via `scripts/sam_attachment_size_probe.py probe --seed 42`.
