...
1. **[RESOLVED — Cycle 2.5] `mod_delta` carries `awarding_agency_code` (CGAC).** Landed live 2026-07-04 (build v19): projected into `delta_out` + BITMAP-indexed (`awarding_agency_code_idx`), non-null on 99.999% of the 25,017,209 mod rows. Agency-aware kinetic scoring no longer needs a `mod_delta ⋈ prime_award_state` join — CGAC binds directly.
2. **[RESOLVED — Cycle 2.5] `mod_delta` carries `award_pool`** (`'parent'` for IDV-vehicle mods, `'child'` for order/definitive mods), BITMAP-indexed (`award_pool_idx`): parent 3,554,249 + child 21,462,960 = mods_out. Percentiles are computed *within* `(CGAC × award_pool)` — a clean two-column BITMAP pushdown; this is what kills the DoD $81.3M artifact.
