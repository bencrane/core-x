...
| Marked content (`content_marking` ≠ []) | 42,307 chunks / 433 resources / 345 awards (288 resources inside the LLM input; values degenerate `['unspecified']` only). **Post-Phase-0 full-body promotion: 2,511 resources corpus-wide / 2,279 inside the P2 input — bracketed out of LLM processing per PHASE 2 DECISION 2026-06-12** |
...
> **DECISION — 2026-06-12 (operator, binding): marked-resource bracket + zero-marginal-spend engine.**
> 1. **Marked bracket.** Every resource with any chunk whose `content_marking` is non-empty (post-Phase-0 full-body promotion: **2,279 inside this phase's input set; 2,511 corpus-wide**) is bracketed out of **all** LLM processing. Marked-document text never enters any agent/LLM context — in-session agent reading counts as egress. The P0/P1 record stays clean: all full-text processing to date was local deterministic compute.
> 2. **Worklist.** The executed P2 worklist = the input set defined below MINUS marked resources ≈ **15,570 unmarked resources** — measured at select time, never assumed.
> 3. **Ledger-recorded, not runtime-filtered.** The bracket is written explicitly to `govcon_requirements_extract_ledger_90day` as `llm_state='excluded_marked'` (enum extended in §5), so coverage accounting stays honest and the exclusion is reversible.
> 4. **Impact.** **1,501 of 35,002** manifest-exploded awards (**4.3%**) have only-marked input documents and receive no LLM-derived fields (`scope_summary`, `capability_tags`, LLM requirement rows); their Phase-1 regex rows remain.
> 5. **Deferral, reversible.** The marked lane is deferred, not dropped — reversible later via a local-model pass that flips `excluded_marked → pending` under the existing harness.
> 6. **Engine constraint (zero marginal spend).** No external API calls for extraction — no Anthropic API/Batch API usage, no GPU rental. The pilot extraction engine is this session's own agents reading staged task files. The harness is engine-agnostic: deterministic select/stage → opaque extraction step producing result JSON files → deterministic validate/land.
> This decision supersedes, for this build: the "288 in-input head-marked → self-hosted or `marked_local_only`" CUI routing, the unmarked-remainder Batch-API policy fork, and the $350–450 batch cost line (both retained below for the record).

**Input (one set, stated once, supersedes any other phrasing):** all scope-sink resources ∪ unknown lexicon-hit ∪ pricing ∪ Phase-1 regex-hit residue = the canonical 17,849 plus the measured regex-hit lexicon-miss unknowns, **minus marked resources per the 2026-06-12 DECISION above → executed worklist ≈ 15,570 unmarked (measured at select time)**. Scope-sink docs enter **unconditionally** — SOW/PWS documents are the substance; gating them on lexicon or regex hits would strip `scope_summary`/`capability_tags` from exactly the "do X" leg this phase exists to ship. Temperature 0, schema-constrained JSON, enums locked, prompt hash in `extractor`. **Idempotency mechanism (pick one, this is it):** scoped `ds.delete("resource_id == '<id>'")` for the resource's prior LLM-lane rows before each merge — LLM `value_norm` drifts across runs, so content-hash ids alone cannot prevent stale-row accumulation; the delete-then-merge plus ledger run stamping makes the crash window between requirements-write and ledger-write harmless. Per-resource token budget caps (top-1% of awards = 25.5% of corpus); p99 docs exceed the 200K context — **sliding-window is mandatory for the tail**, merged on the content-hash key, `coverage_truncated=true` when capped, surfaced in every disclaimer denominator.
