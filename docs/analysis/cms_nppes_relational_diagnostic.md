...
- **Cited cross-references (same date, same method, companion diagnostics):** per-predicate `analyze_plan` pushdown/prune counts and per-index byte sizes for the NPPES analytical layer (`docs/analysis/nppes_analytical_structural_diagnostic.md`); CMS per-family null/NDV and NPI non-pruning (`docs/analysis/cms_open_payments_structural_diagnostic.md`); raw NPPES physical layout (`docs/analysis/nppes_structural_diagnostic.md`).
