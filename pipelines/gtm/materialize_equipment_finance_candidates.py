"""Compute worker — Gen-3 materialization of the equipment_finance_candidates raw landing grain.

Mirrors materialize_equipment_provider / materialize_industries_served. Sources
gtm.equipment_finance_candidates (HQX Postgres) → native Lance at
s3://data-sink/active/equipment_finance_candidates/.

GRAIN / PK. record_id (sha256(domain_norm|linkedin_url_norm|verdict)) — 1:1 copy.
BRIDGE. domain_norm BTREE → firmographics_blitz.domain_norm; linkedin_url_norm BTREE for the
LinkedIn-keyed join lane.

    modal run    pipelines/gtm/materialize_equipment_finance_candidates.py::init_ops
    modal run    pipelines/gtm/materialize_equipment_finance_candidates.py::run           # full overwrite
    modal run    pipelines/gtm/materialize_equipment_finance_candidates.py::append_only   # watermark append
    modal run    pipelines/gtm/materialize_equipment_finance_candidates.py::reindex_only
    modal run    pipelines/gtm/materialize_equipment_finance_candidates.py::verify_only
    modal deploy pipelines/gtm/materialize_equipment_finance_candidates.py
"""
...
DATASET = "equipment_finance_candidates"
DATASET_URI = os.environ.get("EQUIPMENT_FINANCE_CANDIDATES_URI", f"{_ACTIVE}/{DATASET}/")
FEED = "equipment_finance_candidates"
SOURCE_DB = "hqx:gtm.equipment_finance_candidates"
...
INDEXES: dict[str, list[str]] = {
    "BTREE": ["record_id", "domain_norm", "linkedin_url_norm", "company_name"],
    "BITMAP": ["verdict"],
}
...
_NOT_NULL = {"record_id", "company_name", "source", "raw_payload", "landed_at"}
_JSONB_COLS = {"raw_payload"}

OPS_DDL = """
CREATE SCHEMA IF NOT EXISTS ops;

CREATE TABLE IF NOT EXISTS ops.equipment_finance_candidates_runs (
...
CREATE INDEX IF NOT EXISTS equipment_finance_candidates_runs_feed_idx        ON ops.equipment_finance_candidates_runs (feed);
CREATE INDEX IF NOT EXISTS equipment_finance_candidates_runs_status_idx      ON ops.equipment_finance_candidates_runs (status);
CREATE INDEX IF NOT EXISTS equipment_finance_candidates_runs_recorded_at_idx ON ops.equipment_finance_candidates_runs (recorded_at DESC);
"""
...
app = modal.App("equipment-finance-candidates", image=image)
...
def _sql(where: str = "") -> str:
    ...
    return f"""
        ...
        FROM hqx.gtm.equipment_finance_candidates
        {where}
    """
...
def _record_run(mode, rows_total, rows_source, rows_added, watermark, status, error,
    ...
    try:
        with psycopg.connect(dsn) as conn, conn.cursor() as cur:
            ...
            cur.execute(
                """
                INSERT INTO ops.equipment_finance_candidates_runs
...
def ingest_equipment_finance_candidates(trigger_callback_url: str | None = None) -> dict:
    ...
    try:
        ...
        try:
            ...
            rows_source = con.sql("SELECT count(*) FROM hqx.gtm.equipment_finance_candidates").fetchone()[0]
    ...
    if status != "success":
        raise RuntimeError(f"equipment_finance_candidates materialization failed: {error}")
    return {"feed": FEED, "mode": "overwrite", "rows_total": rows_total,
            "rows_source": rows_source, "status": status}


@app.function(secrets=_SECRETS, timeout=60 * 30, memory=16384, cpu=4.0)
def append_equipment_finance_candidates(trigger_callback_url: str | None = None) -> dict:
    import datetime as dt
    ...
    try:
        ...
        try:
            _attach(con, _hqx_dsn())
            rows_source = con.sql(
                f"SELECT count(*) FROM hqx.gtm.equipment_finance_candidates {where}").fetchone()[0]
    ...
    if status != "success":
        raise RuntimeError(f"equipment_finance_candidates append failed: {error}")
...
def apply_ops_ddl() -> dict:
    ...
    with psycopg.connect(dsn) as conn, conn.cursor() as cur:
        ...
        cur.execute("""
            ...
            WHERE table_schema='ops' AND table_name='equipment_finance_candidates_runs' ORDER BY ordinal_position
        """)
        cols = [r[0] for r in cur.fetchall()]
    print(f"ops.equipment_finance_candidates_runs ready — columns: {cols}")
    return {"table": "ops.equipment_finance_candidates_runs", "columns": cols}
...
def run() -> None:
    import json
    print(json.dumps(ingest_equipment_finance_candidates.remote(trigger_callback_url=None), indent=2, default=str))


@app.local_entrypoint()
def append_only() -> None:
    import json
    print(json.dumps(append_equipment_finance_candidates.remote(trigger_callback_url=None), indent=2, default=str))
