"""Materialize ``gtm.equipment_yard_industries_served`` (HQX Postgres) → Lance SoR on R2.

The per-yard website research payloads landed by the edge_api raw lander
(``/api/v1/equipment/industries-served/land``) — one row per landed payload,
...
Run (in-session scale):
    doppler run -p core-x -c prd -- \
        python3 pipelines/gtm/materialize_equipment_yard_industries_served.py
"""
...
DATASET_URI = "s3://data-sink/active/equipment_yard_industries_served/"
...
SELECT record_id,
       ...
       landed_at
FROM hqx.gtm.equipment_yard_industries_served
...
def main() -> None:
    ...
    src_ct = con.execute(
        "SELECT count(*) FROM hqx.gtm.equipment_yard_industries_served"
    ).fetchone()[0]
