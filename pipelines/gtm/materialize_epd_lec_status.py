"""Compute worker — Gen-3 materialization of the epd_lec_status raw landing grain.

Mirrors materialize_industries_served / materialize_equipment_catalog. Sources gtm.epd_lec_status
(HQX Postgres) → native Lance at s3://data-sink/active/epd_lec_status/.
...
BRIDGE. domain_norm BTREE → firmographics_blitz.domain_norm.
    ...
    modal run    pipelines/gtm/materialize_epd_lec_status.py::init_ops
    modal run    pipelines/gtm/materialize_epd_lec_status.py::run           # full overwrite
    modal run    pipelines/gtm/materialize_epd_lec_status.py::append_only   # watermark append
    modal run    pipelines/gtm/materialize_epd_lec_status.py::reindex_only
    modal run    pipelines/gtm/materialize_epd_lec_status.py::verify_only
    modal deploy pipelines/gtm/materialize_epd_lec_status.py
"""
...
DATASET = "epd_lec_status"
DATASET_URI = os.environ.get("EPD_LEC_STATUS_URI", f"{_ACTIVE}/{DATASET}/")
FEED = "epd_lec_status"
SOURCE_DB = "hqx:gtm.epd_lec_status"
...
INDEXES: dict[str, list[str]] = {
    ...
    "BITMAP": ["confidence", "epd_lec_status"],
}
...
_INT64_COLS: set[str] = set()

OPS_DDL = """
CREATE SCHEMA IF NOT EXISTS ops;

CREATE TABLE IF NOT EXISTS ops.epd_lec_status_runs (
...
CREATE INDEX IF NOT EXISTS epd_lec_status_runs_feed_idx        ON ops.epd_lec_status_runs (feed);
CREATE INDEX IF NOT EXISTS epd_lec_status_runs_status_idx      ON ops.epd_lec_status_runs (status);
CREATE INDEX IF NOT EXISTS epd_lec_status_runs_recorded_at_idx ON ops.epd_lec_status_runs (recorded_at DESC);
"""
...
app = modal.App("epd-lec-status", image=image)
...
def _record_run(mode, rows_total, rows_source, rows_added, watermark, status, error,
    ...
    try:
        with psycopg.connect(dsn) as conn, conn.cursor() as cur:
            ...
            cur.execute(
                """
                INSERT INTO ops.epd_lec_status_runs
...
def ingest_epd_lec_status(trigger_callback_url: str | None = None) -> dict:
    ...
    try:
        ...
        try:
            ...
            rows_source = con.sql("SELECT count(*) FROM hqx.gtm.epd_lec_status").fetchone()[0]
    ...
    if status != "success":
        raise RuntimeError(f"epd_lec_status materialization failed: {error}")
    return {"feed": FEED, "mode": "overwrite", "rows_total": rows_total,
            "rows_source": rows_source, "status": status}


@app.function(secrets=_SECRETS, timeout=60 * 30, memory=16384, cpu=4.0)
def append_epd_lec_status(trigger_callback_url: str | None = None) -> dict:
    import datetime as dt
    ...
    try:
        ...
        try:
            _attach(con, _hqx_dsn())
            rows_source = con.sql(
                f"SELECT count(*) FROM hqx.gtm.epd_lec_status {where}").fetchone()[0]
    ...
    if status != "success":
        raise RuntimeError(f"epd_lec_status append failed: {error}")
...
def apply_ops_ddl() -> dict:
    ...
    with psycopg.connect(dsn) as conn, conn.cursor() as cur:
        ...
        cur.execute("""
            ...
            WHERE table_schema='ops' AND table_name='epd_lec_status_runs' ORDER BY ordinal_position
        """)
        cols = [r[0] for r in cur.fetchall()]
    print(f"ops.epd_lec_status_runs ready — columns: {cols}")
    return {"table": "ops.epd_lec_status_runs", "columns": cols}
...
def run() -> None:
    import json
    print(json.dumps(ingest_epd_lec_status.remote(trigger_callback_url=None), indent=2, default=str))


@app.local_entrypoint()
def append_only() -> None:
    import json
    print(json.dumps(append_epd_lec_status.remote(trigger_callback_url=None), indent=2, default=str))
