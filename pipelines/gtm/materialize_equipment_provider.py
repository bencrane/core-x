"""Compute worker — Gen-3 materialization of the equipment_provider raw landing grain.

Mirrors materialize_equipment_catalog / materialize_industries_served. Sources gtm.equipment_provider
(HQX Postgres) → native Lance at s3://data-sink/active/equipment_provider/.
...
BRIDGE. domain_norm BTREE → firmographics_blitz.domain_norm.
    ...
    modal run    pipelines/gtm/materialize_equipment_provider.py::init_ops
    modal run    pipelines/gtm/materialize_equipment_provider.py::run           # full overwrite
    modal run    pipelines/gtm/materialize_equipment_provider.py::append_only   # watermark append
    modal run    pipelines/gtm/materialize_equipment_provider.py::reindex_only
    modal run    pipelines/gtm/materialize_equipment_provider.py::verify_only
    modal deploy pipelines/gtm/materialize_equipment_provider.py
"""
...
DATASET = "equipment_provider"
DATASET_URI = os.environ.get("EQUIPMENT_PROVIDER_URI", f"{_ACTIVE}/{DATASET}/")
FEED = "equipment_provider"
SOURCE_DB = "hqx:gtm.equipment_provider"
...
INDEXES: dict[str, list[str]] = {
    ...
    "BITMAP": ["is_equipment_provider", "mode", "confidence"],
}
...
_JSONB_COLS = {"steps_taken", "raw_payload"}
_BOOL_COLS = {"is_equipment_provider"}

OPS_DDL = """
CREATE SCHEMA IF NOT EXISTS ops;

CREATE TABLE IF NOT EXISTS ops.equipment_provider_runs (
...
CREATE INDEX IF NOT EXISTS equipment_provider_runs_feed_idx        ON ops.equipment_provider_runs (feed);
CREATE INDEX IF NOT EXISTS equipment_provider_runs_status_idx      ON ops.equipment_provider_runs (status);
CREATE INDEX IF NOT EXISTS equipment_provider_runs_recorded_at_idx ON ops.equipment_provider_runs (recorded_at DESC);
"""
...
app = modal.App("equipment-provider", image=image)
...
def _sql(where: str = "") -> str:
    ...
    return f"""
        ...
        FROM hqx.gtm.equipment_provider
        {where}
    """
...
def _record_run(mode, rows_total, rows_source, rows_added, watermark, status, error,
    ...
    try:
        with psycopg.connect(dsn) as conn, conn.cursor() as cur:
            ...
            cur.execute(
                """
                INSERT INTO ops.equipment_provider_runs
...
def ingest_equipment_provider(trigger_callback_url: str | None = None) -> dict:
    ...
    try:
        ...
        try:
            ...
            rows_source = con.sql("SELECT count(*) FROM hqx.gtm.equipment_provider").fetchone()[0]
    ...
    if status != "success":
        raise RuntimeError(f"equipment_provider materialization failed: {error}")
    return {"feed": FEED, "mode": "overwrite", "rows_total": rows_total,
            "rows_source": rows_source, "status": status}


@app.function(secrets=_SECRETS, timeout=60 * 30, memory=16384, cpu=4.0)
def append_equipment_provider(trigger_callback_url: str | None = None) -> dict:
    import datetime as dt
    ...
    try:
        ...
        try:
            _attach(con, _hqx_dsn())
            rows_source = con.sql(
                f"SELECT count(*) FROM hqx.gtm.equipment_provider {where}").fetchone()[0]
    ...
    if status != "success":
        raise RuntimeError(f"equipment_provider append failed: {error}")
...
def apply_ops_ddl() -> dict:
    ...
    with psycopg.connect(dsn) as conn, conn.cursor() as cur:
        ...
        cur.execute("""
            ...
            WHERE table_schema='ops' AND table_name='equipment_provider_runs' ORDER BY ordinal_position
        """)
        cols = [r[0] for r in cur.fetchall()]
    print(f"ops.equipment_provider_runs ready — columns: {cols}")
    return {"table": "ops.equipment_provider_runs", "columns": cols}
...
def run() -> None:
    import json
    print(json.dumps(ingest_equipment_provider.remote(trigger_callback_url=None), indent=2, default=str))


@app.local_entrypoint()
def append_only() -> None:
    import json
    print(json.dumps(append_equipment_provider.remote(trigger_callback_url=None), indent=2, default=str))
