...
sam_opps_attachment_manifest_winners.py), gates to ``access_level='public'`` distinct
...
Full daemonized run:
    doppler run --project core-x --config prd -- \
      ...
      python pipelines/sam_gov/sam_attachment_download.py --daemon --resume

Calibration smoke (200 files, foreground, throwaway URIs):
    ... python pipelines/sam_gov/sam_attachment_download.py --max-files 200 \
        --ledger-uri  s3://data-sink/active/_smoke_files/ \
        --blob-prefix s3://data-sink/active/_smoke_blobs/ \
        --worklist-uri s3://data-sink/active/_smoke_worklist/
"""
...
CKPT_PATH = os.environ.get("SAM_CKPT", "/tmp/sam_ckpt.jsonl")
LOG_PATH = os.environ.get("SAM_LOG", "/tmp/sam_download.log")
...
def run_download(*, storage_options: dict, dsn: str | None, manifest_uri: str = MANIFEST_URI,
    ...
    try:
        ...
        elapsed = max(1e-9, time.monotonic() - t_first) if t_first else 0.0
        mbps = (c["bytes"] / 1e6) / elapsed if elapsed else 0.0
        stats = {"run_id": run_id, "scope": f"winners; {_GATE}", "concurrency": concurrency,
...
def _cli() -> None:
    ...
    run_id = a.run_id or f"download-{dt.datetime.now(dt.timezone.utc):%Y%m%dT%H%M%SZ}"
    out = run_download(
